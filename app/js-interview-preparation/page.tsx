...
// const categories = [
//   { name: "Tricky JS", key: "tricky" },
//   { name: "Polyfill", key: "polyfill" },
//   { name: "Async & Promises", key: "async" },
//   { name: "Closures", key: "closures" },
...
//   {
//     id: 3,
//     title: "What is a Closure? Give an example.",
//     difficulty: "Medium",
//     time: "15 mins",
...
//   const [selectedCategory, setSelectedCategory] = useState("tricky")
...
import { 
    BookOpen, Zap, Code, Shield, Brain, Loader, LayoutDashboard, Menu, X, LucideIcon, LogIn, CheckCircle,
    ChevronDown, ChevronUp, Filter, Search, BarChart3
} from 'lucide-react';


// Load Tailwind CSS automatically
// The use of Lucide React is assumed to be available in this environment.

// --- 1. Data Structures and Content Definitions ---

// Map string names to Lucide components for dynamic rendering
const IconMap: { [key: string]: LucideIcon } = {
  BookOpen, Zap, Code, Shield, Brain, Loader, LayoutDashboard, Menu, X, LogIn, CheckCircle,
  ChevronDown, ChevronUp, Filter, Search, BarChart3
};
...
interface Topic {
  id: string;
  title: string;
  category: 'Core JS' | 'React/Patterns' | 'Special Focus' | 'Dashboard';
  path: string;
  iconName: string; 
  content: string;
  hasQuestions: boolean;
  filterCategories?: string[]; // Specific categories from the QUESTIONS array to filter by
}
...
const TOPICS: Topic[] = [
  {
    ...
    hasQuestions: false,
    content: `
      <h2 class="text-3xl font-bold mb-4 text-indigo-700">JS & React Interview Kit</h2>
      <p class="text-gray-600 mb-6">Welcome to your preparation hub! Select a topic from the sidebar to dive into the focused **Question Banks**.</p>

      <div class="grid md:grid-cols-2 gap-4">
        <div class="p-4 bg-indigo-50 rounded-xl shadow-inner border border-indigo-200">
          <h3 class="font-semibold text-lg text-indigo-600 flex items-center"><BookOpen className="w-5 h-5 mr-2"/> Core JS Concepts</h3>
          <p class="text-sm text-gray-700 mt-2">Practice **Closures**, **ES6**, **Prototypes**, and **Data Structures**. Use the filters to target Beginner, Intermediate, or Advanced levels.</p>
        </div>
        <div class="p-4 bg-yellow-50 rounded-xl shadow-inner border border-yellow-200">
          <h3 class="font-semibold text-lg text-yellow-600 flex items-center"><Zap className="w-5 h-5 mr-2"/> Must-Know Challenges</h3>
          <p class="text-sm text-gray-700 mt-2">Master **Asynchronous JavaScript**, tricky **Coercion**, and advanced topics like **Debouncing/Throttling**.</p>
        </div>
      </div>
    `,
  },
  // --- CORE JS TOPICS (Q&A Banks) ---
  {
    id: 'closures',
    title: 'Closures & Scope (Q&A)',
    category: 'Core JS',
    iconName: 'BookOpen',
    path: '/interview-kit/topics/closures',
    hasQuestions: true,
    content: 'Interactive Question Bank',
    filterCategories: ['Scope', 'Functions', 'OOP', 'Types', 'ES6', 'Objects', 'Arrays', 'Browser APIs'],
  },
  {
    id: 'async',
    title: 'Async JavaScript (Q&A)',
    category: 'Core JS',
    iconName: 'Loader',
    path: '/interview-kit/topics/async',
    hasQuestions: true,
    content: 'Interactive Question Bank',
    filterCategories: ['Async', 'DOM', 'Performance', 'Error Handling'],
  },
  {
    id: 'tricky',
    title: 'Tricky JS Questions (Q&A)',
    category: 'Special Focus',
    iconName: 'Zap',
    path: '/interview-kit/tricky',
    hasQuestions: true,
    content: 'Interactive Question Bank',
    filterCategories: ['Operators', 'FP', 'Tooling', 'General', 'Security', 'Design Patterns', 'TypeScript'],
  },
  // --- OTHER STATIC CONTENT TOPICS ---
  {
    id: 'patterns',
    title: 'Design Patterns',
    category: 'React/Patterns',
    iconName: 'Code',
    path: '/interview-kit/patterns',
    hasQuestions: false,
    content: `
      <h3 class="text-2xl font-bold mb-3 text-indigo-700">Design Patterns & SOLID Principles</h3>
      <p class="mb-4">This section covers common JavaScript design patterns (Module, Factory, Observer) and how to apply SOLID principles to frontend architecture.</p>
    `,
  },
  {
    id: 'metrics',
    ...
    category: 'Special Focus',
    iconName: 'BarChart3',
    path: '/interview-kit/metrics',
    hasQuestions: false,
    content: `
      <h3 class="text-2xl font-bold mb-3 text-yellow-700">Web Vitals and Runtime Performance</h3>
      <p class="mb-4">Understand key metrics like FCP, LCP, and CLS, and how to use browser tools (Lighthouse, DevTools) to measure and debug performance bottlenecks in your JavaScript and React applications.</p>
    `,
  },
];
...
//     { id: 62, question: 'What is property access using bracket notation vs. dot notation?', category: 'Objects', level: 'Intermediate', answer: 'Dot notation (`obj.prop`) is cleaner but requires the property name to be a fixed, valid identifier. Bracket notation (`obj[\'prop\']` or `obj[variable]`) is required when the property name is stored in a **variable** (dynamic access), contains **spaces** or special characters, or is a **reserved word**.' },
...
const getTopicByPath = (path: string): Topic | undefined => {
  return TOPICS.find(topic => topic.path === path);
};

// --- 2. Shared Components (Sidebar, RightSidebar) ---
...
const NavLink: React.FC<{ topic: Topic, isActive: boolean, onClick: () => void }> = ({ topic, isActive, onClick }) => {
  const IconComponent = IconMap[topic.iconName] || BookOpen;
  
  return (
    <a
      onClick={(e) => { e.preventDefault(); onClick(); }}
      href={topic.path} // Placeholder for display/SEO
      className={`
        w-full text-left flex items-center px-4 py-3 border-b border-gray-100 last:border-b-0
...
const Sidebar: React.FC<SidebarProps> = ({ currentPath, onNavigate }) => {
  const categories = useMemo(() => {
    // Filter out the Dashboard topic before grouping for the categories list
    const navigableTopics = TOPICS.filter(t => t.id !== 'dashboard');
    return navigableTopics.reduce((acc, topic) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-gray-200 shadow-sm overflow-y-auto">
      <div className="p-4 border-b border-gray-200">
        <h2 className="text-xl font-bold text-gray-800">
          <span className="text-indigo-600">JS</span> Interview Kit
        </h2>
      </div>

      {/* Dashboard Link (Always at the top) */}
      <nav className="border-b border-gray-200">
          <NavLink 
            topic={getTopicByPath('/interview-kit') as Topic}
            isActive={currentPath === '/interview-kit'}
            onClick={() => onNavigate('/interview-kit')}
          />
      </nav>
      ...
      {Object.entries(categories).map(([category, topics]) => (
        <React.Fragment key={category}>
          <h3 className="text-xs uppercase font-bold tracking-wider text-gray-500 mt-4 mb-1 px-4">
            {category}
          </h3>
          <nav className="space-y-0.5">
            {topics.map(topic => (
              <NavLink 
                key={topic.id} 
                topic={topic} 
                isActive={currentPath === topic.path} 
...
// --- 3. Question Bank Component (Specialized Content) ---
import QUESTIONS from '@/app/data/quetions';
...
const QuestionBank: React.FC<QuestionBankProps> = ({ filterCategories, topicTitle }) => {
    ...
    // Helper function for level tag styling
    ...
    const allQuestionsInTopic = useMemo(() => {
        return QUESTIONS.filter(q => filterCategories.includes(q.category));
    }, [filterCategories]);


    const filteredQuestions = useMemo(() => {
        let results = allQuestionsInTopic;

        // 1. Filter by Level
        if (filterLevel !== 'All') {
            results = results.filter(q => q.level === filterLevel);
        }
        ...
        return results;
    }, [allQuestionsInTopic, filterLevel, searchQuery]);

    return (
        <div className="p-6 sm:p-8 overflow-y-auto h-full space-y-6">
            <h1 className="text-3xl font-extrabold text-gray-800 mb-2">
                {topicTitle} ({filteredQuestions.length} of {allQuestionsInTopic.length})
            </h1>
            <p className="text-gray-600">Filter by level or search by keyword to target your study areas within this topic.</p>

            {/* Filter and Search Bar */}
            <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4 p-4 bg-white rounded-xl shadow-md border border-gray-100">
                ...
                <div className="flex space-x-2 flex-shrink-0">
                    {(['All', 'Beginner', 'Intermediate', 'Advanced'] as const).map(level => (
                        <button
                            ...
                            className={`
                                px-3 py-1 text-sm font-semibold rounded-full border transition duration-150
                                ${filterLevel === level 
            ...
            <div className="space-y-4">
                {filteredQuestions.map((q) => (
                 <div key={q.id} className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden transition duration-300 hover:shadow-xl">
  <div 
      className="p-4 cursor-pointer flex justify-between items-center hover:bg-gray-50 transition"
      onClick={() => toggleExpand(q.id)}
  >
      <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3">
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full border ${getLevelColor(q.level)}`}>
              {q.level}
          </span>
  ...
  {expandedId === q.id && (
      <div className="p-4 border-t border-gray-200 bg-gray-50">
          ...
          <p className="text-gray-600 leading-relaxed mb-2">{q.answer}</p>
          {q.example && (
              <pre className="bg-gray-100 p-3 rounded text-sm text-gray-800 overflow-x-auto">
                ...
                {filteredQuestions.length === 0 && (
                    <div className="p-10 text-center text-gray-500 border-dashed border-2 rounded-xl mt-6">
                        No questions found matching the current filters or search query.
                    </div>
                )}
            </div>
        </div>
...
// --- 4. Main Application Component (The 3-Column Layout) ---

const ContentArea: React.FC<{ topic: Topic }> = ({ topic }) => {
    const TopicIcon = IconMap[topic.iconName] || BookOpen;
    
    // Render the specialized Question Bank component if the flag is set and filterCategories exist
    if (topic.hasQuestions && topic.filterCategories) {
        return <QuestionBank filterCategories={topic.filterCategories} topicTitle={topic.title} />;
    }

    // Render static content otherwise
    // Determine header color based on category
    const headerColor = topic.category === 'Special Focus' ? 'text-red-700' : 'text-indigo-700';

    return (
        <div className="p-6 sm:p-8 overflow-y-auto h-full">
            <header className="mb-6 pb-4 border-b border-gray-200">
                <div className={`flex items-center space-x-3 text-sm font-medium ${headerColor}`}>
                    <TopicIcon className="w-5 h-5"/>
                    <span className="uppercase tracking-wider">{topic.category}</span>
                </div>
                <h1 className="text-3xl font-extrabold text-gray-800 mt-1">{topic.title}</h1>
            </header>
            ...
            <footer className="mt-10 pt-6 border-t border-gray-100 text-sm text-gray-500 italic">
                <p>✔ Comprehensive coverage of modern JavaScript interviews.</p>
            </footer>
        </div>
    );
};
...
export default function App() {
    // State to simulate routing/page selection, defaults to Dashboard
    const [currentPath, setCurrentPath] = useState<string>(TOPICS[0].path); 

    const currentTopic = useMemo(() => {
        return getTopicByPath(currentPath) || TOPICS[0];
    }, [currentPath]);
    ...
    return (
        <div className="h-screen w-screen flex bg-gray-50 font-sans">
            ...
            {/* Sidebar (Left Column) - Fixed Width, visible on large screens */}
            <div className="hidden lg:block w-72 flex-shrink-0 h-full">
                <Sidebar currentPath={currentPath} onNavigate={handleNavigate} />
            </div>

            {/* Main Content Area (Center Column) - Fluid Width */}
            <main className="flex-grow min-w-0 overflow-y-auto">
                <div className="flex h-full">
                    {/* Centered Content Container */}
                    <div className="flex-grow min-w-0 max-w-4xl mx-auto h-full bg-white shadow-xl">
                        <ContentArea topic={currentTopic} />
                    </div>
                </div>
            </main>
            ...
            <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 z-10 shadow-lg">
                <p className="text-sm text-center text-gray-600">Use the desktop view to access the full sidebar navigation and Q&A filters.</p>
            </div>
