...
import useSWR from "swr";
import { useAuth } from "@/components/auth-provider";
import CourseCardStudent from "@/components/students/courseCard";
    ...
    <div className="space-y-8">
      ...
      {courses.length > 0 ? (
        <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map(course => (
            <CourseCardStudent key={course._id} course={course} />
          ))}
        </div>
      ) : (
        <div className="text-center py-16">
          <p className="text-lg font-semibold text-gray-700">You haven't started any courses yet!</p>
          <p className="text-gray-500 mt-2">Enroll in a course and start learning today.</p>
          <Button className="mt-4 gradient-primary">Explore Courses</Button>
        </div>
      )}
    </div>
  );
